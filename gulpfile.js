...
gulp.task('pre-build', function preBuild() {
  return gulp
    .src('./src/**/*.{ts,js,gs,json,html}')
    .pipe(gulp.dest(`./${argv.buildpart}`));
});
...
gulp.task('preparation-update-client-code', function(cb) {
  return gulp
    .src('./clients/src/**/*.{ts,js,gs,json,html}')
    .pipe(gulp.dest('./clients/build'));
});
...
gulp.task('preparation-update-clients', function(cb) {
  ...
  cmd.on('close', function(code) {
    ...
    const appscript = JSON.parse(
      fs.readFileSync('./clients/src/appsscript.json')
    );
    const claspjson = JSON.parse(fs.readFileSync('./.clasp.json'));
    const index = appscript.dependencies.libraries.findIndex(
      lib => lib.userSymbol === 'Library'
    );
    ...
    appscript.dependencies.libraries[
      index
    ].developmentMode = Object.prototype.hasOwnProperty.call(
      argv,
      'developmentMode'
    );
    ...
    fs.writeFileSync(
      './clients/build/appsscript.json',
      JSON.stringify(appscript, null, '  '),
      { flag: 'w' }
    );
    cb(code);
  });
...
gulp.task('update-clients-bulk', function(cb) {
  const projects = JSON.parse(
    /start[\s\S]*?`([\s\S]+?)`[\s\S]*?end/.exec(
      fs.readFileSync(`./settings/${argv.part}/assets/index.js`)
    )[1]
  );
  // cb(projects);
  const clientsList = argv['clients-list']
    ? JSON.parse(`[${argv['clients-list']}]`)
    : undefined;
...
gulp.task('update-version', async function(cb) {
  ...
  for await (const data of abbrevRef.stdout)
    version.push(data.toString().replace(/[\r\n]/g, ''));
  for await (const data of short.stdout)
    version.push(data.toString().replace(/[\r\n]/g, ''));
  return cb(0);
});
...
gulp.task('readversion', function(cb) {
  ...
  cmd.on('close', function(code) {
    ...
    const appscript = JSON.parse(
      fs.readFileSync('./scripts/src/appsscript.json')
    );
    const index = appscript.dependencies.libraries.findIndex(
      lib => lib.userSymbol === 'Luxcom'
    );
    console.log(`Lib is ${!~index ? 'NOT ' : ''}DETECTED`);
    appscript.dependencies.libraries[index].version = version;
    fs.writeFileSync(
      './scripts/src/appsscript.json',
      JSON.stringify(appscript, null, '  ')
    );
    cb(code);
  });
...
gulp.task('readversion-prod', function(cb) {
  ...
  cmd.on('close', function(code) {
    ...
    appscript.dependencies.libraries[index].version = version;
    appscript.dependencies.libraries[index].libraryId =
      '1fRodYEJFBC3jTabfNabvAUglQo2FxtkZLkQL7I4ulgi1OMZpJLJV8NBu';
    fs.writeFileSync(
      './scripts/src/appsscript.json',
      JSON.stringify(appscript, null, '  ')
    );
